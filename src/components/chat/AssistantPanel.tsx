...
export default function AssistantPanel({ sessionId }: Props) {
  const [messages, setMessages] = useState<ChatMessageType[]>(
    sessionId ? [] : initialChatMessages
  );
      ...
      if (result.success) {
        ...
        setMessages((prev) => [
          ...prev,
          {
            id: `msg-${Date.now()}-err`,
            role: 'assistant',
            content: `I encountered an error: ${result.error.message}. Please try again.`,
            timestamp: ts2,
          },
        ]);
      }
      ...
      <div className="border-b border-[#1a1a1a] px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <span className={[
            ...
            {sessionId ? 'grounded mode' : 'demo mode'}
          </span>
        </div>
      </div>
